    ...
    <div className={`px-4 py-3 shadow-lg rounded-lg border-2 min-w-[200px] ${
      ...
      <div className="flex items-center space-x-2 mb-2">
        ...
        <span className="font-semibold text-sm">
          {data.label || `${data.address.slice(0, 6)}...${data.address.slice(-4)}`}
        </span>
      </div>
      
      <div className="text-xs opacity-80">
        <div>Address: {data.address.slice(0, 8)}...{data.address.slice(-6)}</div>
        {data.balance && (
...
export function WalletFlowChart({ walletAddress, graphId }: WalletFlowChartProps) {
  // Fetch real graph data
  const { data: graphResponse, isLoading, error } = useWalletGraph(walletAddress);
    ...
    if (graphData && graphData.nodes && graphData.nodes.length > 0) {
      ...
      return graphData.nodes.map((node: any, index: number) => ({
        ...
        data: {
          ...
          label: node.walletAddress === walletAddress ? 'Main Wallet' : `Wallet ${index + 1}`,
          isMain: node.walletAddress === walletAddress,
          balance: '0',
          transactionCount: 0,
          tags: [node.nodeType || 'WALLET']
        },
      }));
    }
    return mockNodes;
  ...
  const finalEdges = useMemo(() => {
    if (graphData && graphData.edges && graphData.edges.length > 0) {
      ...
      return graphData.edges.map((edge: any) => ({
        id: `e${edge.id}`,
        source: edge.fromWalletAddress,
        target: edge.toWalletAddress,
        type: 'smoothstep',
        animated: true,
        ...
        <div className="w-full h-full bg-slate-900 rounded-b-lg">
          ...
          >
            ...
            <Background 
              ...
              color="#374151"
            />
            <Panel position="top-right" className="bg-slate-800/90 p-3 rounded-lg border border-slate-600">
